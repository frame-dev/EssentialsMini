...
public class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            Main.getInstance().getLogger4J().log(Level.ERROR, "Error saving settings.yml : " + settingsFile.getAbsolutePath(), ex);
        }

        // API Init
        new EssentialsMiniAPI(this);
        getLogger().info("API loaded and enabled.");

        // Check if HomeTP is enabled in the configuration
        ...
        if (Bukkit.getPluginManager().getPlugin("ProtocolLib") != null) {
            getLogger().info("ProtocolLib detected. Initializing ProtocolLibUtil.");
            // new ProtocolLibUtil().createMotdHover();
        } else {
            getLogger().warning("ProtocolLib not detected. Running without ProtocolLib functionality.");
        }
    }
    ...
    private @NotNull List<String> getComments() {
        ...
        comments.add("Welcome to the Config from the Plugin EssentialsMini");
        comments.add("Prefix features (new key: 'prefix')");
        comments.add("'&' will be replaced with '§'\n");
        comments.add("'>>' will be replaced with '»'\n");
        comments.add("'<<' will be replaced with '«'\n");
        comments.add("'->' will be replaced with '→'\n");
        comments.add("'<-' will be replaced with '←'\n");
